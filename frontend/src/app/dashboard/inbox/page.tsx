        ...
        if (conversations.length === 0) {
            return (
                <div className="flex flex-col items-center justify-center h-full text-white/20 p-12 text-center">
                    <span className="material-symbols-outlined text-[6rem] mb-6 font-thin opacity-10">sensors_off</span>
                    <p className="font-black uppercase tracking-[0.4em] text-xs">No active frequency detected.</p>
                </div>
            );
        }
                                    ...
                                    <div className="relative bg-black border border-white/10 rounded-2xl overflow-hidden shadow-2xl shadow-black focus-within:border-primary/50 transition-all duration-700">
                                        ...
                                        <div className="flex justify-between items-center p-6 bg-white/1">
                                            ...
                                            <motion.button
                                                ...
                                                className="bg-primary text-white px-10 py-4 rounded-full text-[10px] font-black uppercase tracking-[0.4em] shadow-2xl shadow-primary/40 flex items-center gap-4 hover:bg-primary-dark transition-all disabled:opacity-50"
                                            >
                                                {sending ? <Loader2 className="w-4 h-4 animate-spin" /> : <>TRANSMIT <span className="material-symbols-outlined text-sm">send</span></>}
                                            </motion.button>
                                        </div>
                                    </div>
                                </form>
...
export default function InboxPage() {
    return (
        <Suspense fallback={<div className="flex justify-center items-center h-screen bg-background-dark"><div className="w-12 h-12 border-4 border-primary border-t-transparent rounded-full animate-spin" /></div>}>
            <InboxContent />
        </Suspense>
    );
}
